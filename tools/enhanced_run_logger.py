...
class EnhancedRunLogger:
    ...
    def log_frame_with_timing(self, brain_client, robot_state: Optional[Dict] = None, gui_fps: float = None):
        ...
        timing_sample = {
            ...
            'fps': fps,
            'gui_fps': gui_fps,  # Add GUI FPS tracking
        ...
        if robot_state:
            timing_sample['robot_state'] = robot_state
    ...
    def log_performance_summary(self):
        ...
        # Calculate GUI FPS statistics if available
        gui_fps_samples = [s.get('gui_fps', 0) for s in recent_samples if s.get('gui_fps') is not None]
        avg_gui_fps = sum(gui_fps_samples) / len(gui_fps_samples) if gui_fps_samples else None
        ...
        performance_summary = {
            ...
            'avg_fps': avg_fps,
            'avg_gui_fps': avg_gui_fps,
            'avg_frame_time': avg_frame_time,
            'timing_stats': timing_stats,
            'top_time_consumers': self._get_top_time_consumers(timing_stats)
        }
        
        # Add node count progression
        node_counts = [s['brain_nodes'] for s in recent_samples]
        performance_summary['node_count_range'] = {
            'min': min(node_counts),
            'max': max(node_counts),
    ...
    def _print_timing_summary(self, summary: Dict):
        ...
        if summary.get('avg_gui_fps') is not None:
            print(f"   GUI FPS: {summary['avg_gui_fps']:.1f}")
        print(f"   Frame time: {summary['avg_frame_time']:.6f}s")
        
        if 'node_count_range' in summary:
            node_range = summary['node_count_range']
            print(f"   Brain nodes: {node_range['min']}-{node_range['max']} (avg: {node_range['avg']:.0f})")
