...
class MultiDrivePredictor:
    ...
    def __init__(self, max_depth: int = 5, randomness_factor: float = 0.3,
        ...
        self._world_graph_initialized = False
    ...
    def generate_prediction(self, current_context: List[float], 
        ...
        # Initialize GPU predictors in motivation system if not already done
        if not self._world_graph_initialized and world_graph and world_graph.has_nodes():
            self._initialize_motivation_system_predictors(world_graph)
            self._world_graph_initialized = True
    ...
    def _initialize_motivation_system_predictors(self, world_graph):
        """Initialize GPU predictors in all drives that support it."""
        ...
        for drive_name, drive in self.motivation_system.drives.items():
            if hasattr(drive, 'initialize_gpu_predictor'):
                try:
                    ...
                    print(f"   ✅ {drive_name} drive: GPU predictor initialized")
                except Exception as e:
                    print(f"   ⚠️  {drive_name} drive: GPU predictor failed - {e}")
            elif hasattr(drive, 'predictive_novelty_enabled') or hasattr(drive, 'predictive_safety_enabled') or hasattr(drive, 'predictive_mastery_enabled'):
                print(f"   ℹ️  {drive_name} drive: Predictive features available but not initialized")
            else:
                print(f"   ℹ️  {drive_name} drive: Rule-based assessment only")
        
        print(f"🎯 GPU predictor initialization completed!")