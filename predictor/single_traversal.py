...
class SingleTraversal:
    ...
    def _find_most_similar_node(self, target_context: List[float], 
        ...
        # Search all nodes for best similarity match
        for node in world_graph.all_nodes():
            similarity = world_graph._calculate_context_similarity(
                target_context, node.mental_context
            )
            
            # Weight similarity by node strength (stronger memories are more relevant)
            weighted_similarity = similarity * (1.0 + node.strength * 0.1)
