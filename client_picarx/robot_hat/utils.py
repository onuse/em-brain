...
def enable_speaker():
    ...
    # set_default_sink volume to 100
    usr_name = get_username()
    USER_RUN=f"sudo -u {usr_name} env XDG_RUNTIME_DIR=/run/user/$(id -u {usr_name})"
    status, result = run_command(f"{USER_RUN} pactl set-sink-volume @DEFAULT_SINK@ 100%")
    if status != 0:
        error(f"Failed to set_default_sink volume to 100: {result}")
