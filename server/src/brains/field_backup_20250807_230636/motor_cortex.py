...
class MotorCortex:
    ...
    def process_intentions(self, 
        ...
        # OPTIMIZED: Check intention strength without .item() call
        intention_strength_tensor = torch.max(torch.abs(intentions)).detach()
        intention_strength = intention_strength_tensor.item()  # Keep for logging compatibility
        ...
        # OPTIMIZED: Use tensor comparison for GPU efficiency
        if intention_strength_tensor < self.activation_threshold:
